...
def build_copy_defaults(style_group: str) -> dict[str, str]:
    if style_group == "vintage":
        return {
            "logo": "MOONLIGHT CAFE",
            "slogan": "since 2024",
            "headline": "옛 감성 한 잔",
...
def main() -> None:
    ...
    defaults = build_copy_defaults(decision.style_group)
    print(f"\n선택된 템플릿: {decision.template_name} ({decision.style_group})")
    print(f"선택 전략: {decision.strategy}, confidence={decision.confidence:.2f}")
    ...
    if not image_path.exists():
        raise FileNotFoundError(
            ...
            f"절대경로나 sample_ver3 기준 상대경로를 입력해야 합니다."
        )
