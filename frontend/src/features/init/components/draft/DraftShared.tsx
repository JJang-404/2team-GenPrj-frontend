import type { InitProduct, InputData } from '../../types';

// ─── StoreTitle ───────────────────────────────────────────────────────────────
export const StoreTitle = ({ storeName, brandColor, className = '' }: { storeName: string; brandColor: string; className?: string }) => (
  <h3
...
// ─── SloganText ───────────────────────────────────────────────────────────────
export const SloganText = ({ slogan, className = '' }: { slogan: string; className?: string }) =>
  slogan ? (
    <p className={`text-white/80 font-bold tracking-[0.2em] font-zen uppercase z-30 ${className}`}>{slogan}</p>
  ) : null;

// ─── ProductInfo ──────────────────────────────────────────────────────────────
export const ProductInfo = ({ p, isSquare }: { p: InitProduct; isSquare: boolean }) => {
  const hasContent = (p.showName && p.name) || (p.showPrice && p.price) || (p.showDesc && p.description);
  if (!hasContent) return null;
  return (
    <div
      className={`bg-black/50 backdrop-blur-md ${isSquare ? 'p-2 rounded-lg' : 'p-4 rounded-2xl'} text-white z-20 shadow-xl border border-white/10`}
    >
      {p.showName && p.name && (
      ...
      {p.showDesc && p.description && (
        <p className={`${isSquare ? 'text-[10px]' : 'text-[14px]'} font-zen opacity-90 mt-1 leading-tight border-t border-white/20 pt-1`}>
          {p.description}
        </p>
      )}
    </div>
...
export const ImageFrame = ({ activeProducts, styleIdx, isSquare, isFiveFour, isFull = false, className = '' }: ImageFrameProps) => {
  ...
  return (
    <div className={`${gridClass} ${containerHeight} z-0 ${className} ${isFull ? 'w-full h-full' : 'w-full'}`}>
...
export const ExtraInfoStrip = ({ extraInfo, isSquare }: { extraInfo: InitExtraInfo | null; isSquare: boolean }) => {
  if (!extraInfo) return null;

  const badges = [
    extraInfo.showParkingSpaces && extraInfo.parkingSpaces && `주차 ${extraInfo.parkingSpaces}`,
    extraInfo.showPhone   && extraInfo.phone    && extraInfo.phone,
    extraInfo.showAddress && extraInfo.address  && extraInfo.address,
    extraInfo.petFriendly    && extraInfo.showPetFriendly    && '애견 동반 가능',
