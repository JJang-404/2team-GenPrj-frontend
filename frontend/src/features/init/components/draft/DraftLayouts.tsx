...
import type { InitOptions, InitProduct, RatioStyles, InputData } from '../../types';

interface LayoutProps {
  activeProducts: InitProduct[];
  options: InitOptions;
  inputData: InputData;
  ratioStyles: RatioStyles;
}

const useDecorOverlays = (bgType: string) => bgType === 'AI 생성';

// ─── 1. 클래식 ─────────────────────────────────────────────────────────────────
export const ClassicLayout = ({ activeProducts, options, inputData, ratioStyles }: LayoutProps) => {
  ...
  return (
    <div className={`w-full h-full flex flex-col ${containerPadding} items-center justify-start ${showOverlays ? 'bg-white/5' : ''}`}>
      <div className="w-full text-center border-b border-white/10 pb-6 mb-4 z-30">
        <StoreTitle storeName={inputData.storeName} brandColor={options.brandColor} className={`${titleSize} mb-2`} />
        <SloganText slogan={inputData.mainSlogan} className={isSquare ? 'text-[10px]' : 'text-sm'} />
      </div>
      <ImageFrame activeProducts={activeProducts} styleIdx={0} isSquare={isSquare} isFiveFour={isFiveFour} className="flex-1 w-full flex items-center justify-center py-4" />
      <div className={`w-full flex ${isSquare ? 'gap-2' : 'gap-4'} flex-wrap justify-center z-30 pt-4`}>
        {activeProducts.map((p) => <ProductInfo key={p.id} p={p} isSquare={isSquare} />)}
      </div>
    </div>
  );
};
...
// ─── 2. 역동적 ─────────────────────────────────────────────────────────────────
export const DynamicLayout = ({ activeProducts, options, inputData, ratioStyles }: LayoutProps) => {
  ...
  return (
    <div className={`w-full h-full flex flex-col ${containerPadding} relative overflow-hidden ${showOverlays ? 'bg-white/5' : ''}`}>
      ...
      <div className={`absolute ${isSquare ? 'top-6 left-6' : 'top-12 left-12'} z-30`}>
        <StoreTitle storeName={inputData.storeName} brandColor={options.brandColor} className={`${isSquare ? 'text-4xl' : isTall ? 'text-7xl' : 'text-6xl'} -rotate-3 drop-shadow-2xl`} />
        <SloganText slogan={inputData.mainSlogan} className={`mt-2 ml-2 ${isSquare ? 'text-xs' : 'text-xl'} opacity-60`} />
      </div>
      <div className="flex-1 flex items-center justify-center mt-20">
        <ImageFrame activeProducts={activeProducts} styleIdx={1} isSquare={isSquare} isFiveFour={isFiveFour} className="w-[98%] h-[75%] scale-110" />
      </div>
      <div className="absolute bottom-8 right-8 flex flex-col gap-3 items-end z-30">
        {activeProducts.slice(0, 3).map((p) => <ProductInfo key={p.id} p={p} isSquare={isSquare} />)}
      </div>
    </div>
  );
};
...
// ─── 3. 중앙 집중 ─────────────────────────────────────────────────────────────
export const FocusedLayout = ({ activeProducts, options, inputData, ratioStyles }: LayoutProps) => {
  const { isSquare, isFiveFour, containerPadding } = ratioStyles;
  const showOverlays = useDecorOverlays(options.bgType);
  const firstP = activeProducts[0];
  const priceText = firstP?.price ? `${Number(firstP.price).toLocaleString()} ${firstP.currency ?? '원'}` : null;

  return (
    <div className={`w-full h-full flex flex-col ${containerPadding} items-center justify-between relative ${showOverlays ? 'bg-black/5' : ''}`}>
      ...
      {firstP?.showPrice && priceText && (
        <div className="absolute right-4 top-1/2 -translate-y-1/2 rotate-90 origin-center z-30 whitespace-nowrap">
          <p className={`${isSquare ? 'text-lg' : 'text-3xl'} font-bold font-zen text-yellow-400/80 tracking-tighter drop-shadow-lg`}>{priceText}</p>
        </div>
      )}
      <div className="flex-1 w-full flex items-center justify-center z-10 min-h-0">
        <ImageFrame activeProducts={activeProducts} styleIdx={2} isSquare={isSquare} isFiveFour={isFiveFour} className="scale-110" />
      </div>
      <div className="w-full flex flex-col items-center z-30 mt-auto pb-10">
        ...
        <div className="text-center w-full px-4 break-words">
          <StoreTitle storeName={inputData.storeName} brandColor={options.brandColor} className={isSquare ? 'text-xl' : isFiveFour ? 'text-4xl' : 'text-6xl'} />
...
// ─── 4. 풀 이머시브 ──────────────────────────────────────────────────────────
export const ImmersiveLayout = ({ activeProducts, options, inputData, ratioStyles }: LayoutProps) => {
  ...
  return (
    <div className="w-full h-full relative flex flex-col items-center justify-center overflow-hidden">
      <div className={`absolute inset-0 z-0 flex items-center justify-center p-4 ${showOverlays ? 'opacity-80 scale-115' : ''}`}>
        <ImageFrame activeProducts={activeProducts} styleIdx={3} isSquare={isSquare} isFiveFour={isFiveFour} isFull />
      </div>
      <div className="z-30 text-center px-6">
        <StoreTitle
          storeName={inputData.storeName}
          brandColor={options.brandColor}
          className={`${isSquare ? 'text-5xl' : isTall ? 'text-8xl' : 'text-7xl'} drop-shadow-[0_10px_60px_rgba(0,0,0,1)] mb-4`}
        />
        <SloganText
          slogan={inputData.mainSlogan}
          className={`${isSquare ? 'text-sm' : 'text-lg'} bg-black/70 px-8 py-3 rounded-full backdrop-blur-2xl inline-block border border-white/20 shadow-2xl`}
        />
      </div>
      <div className={`absolute ${isSquare ? 'bottom-6 right-6' : 'bottom-12 right-12'} flex flex-col gap-2 items-end z-30`}>
        {activeProducts.slice(0, 2).map((p) => <ProductInfo key={p.id} p={p} isSquare={isSquare} />)}
      </div>
    </div>
  );
};
