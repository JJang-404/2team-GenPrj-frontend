...
const ExtraInfoSection = ({ extraInfo, updateExtraInfo }) => {
  ...
  return (
    <section className="space-y-3">
      ...
      {/* 좌석 공간 수 */}
      <Row label="좌석 공간 수" visible={extraInfo.showSeatCount} onToggle={upd('showSeatCount')}>
        <input
          type="number"
          value={extraInfo.seatCount}
          onChange={(e) => updateExtraInfo('seatCount', parseInt(e.target.value) || 0)}
          placeholder="0"
          className={numCls}
        />
      ...
      <CheckRow
        id="hasDelivery"
        label="배달 플랫폼 기능 여부"
        checked={extraInfo.hasDelivery}
        onCheck={(v) => updateExtraInfo('hasDelivery', v)}
        visible={extraInfo.showDelivery}
        onToggle={upd('showDelivery')}
      />
