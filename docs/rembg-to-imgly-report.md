...
기존에는 배경 제거 파이프라인(배경 제거 → 크롭 → blob 정리 → 슬롯 크기 재계산)이 UI 컴포넌트 내부에 인라인으로 작성되어 있었음. 이로 인해:
- UI 컴포넌트와 비즈니스 로직이 결합되어 다른 브랜치에 재사용 불가
...
frontend/src/
├─ config/
│   └─ backgroundRemoval.js   ← WebGPU 설정 싱글톤
├─ utils/
│   ├─ cropToBoundingBox.js   ← 바운딩 박스 크롭 (순수 Canvas 유틸)
│   └─ removeBackground.js    ← 파이프라인 오케스트레이션 + 슬롯 크기 계산
├─ main.jsx                    ← preload(BG_REMOVAL_CONFIG) 호출
└─ hooks/
    └─ useProducts.js          ← UI hook, removeBgPipeline import하여 사용
```
...
## 8. `feature/initPage` 브랜치 포팅

### 8.1 브랜치 간 구조 차이

| 항목 | `feature/demo_with_img_save` | `feature/initPage` |
|------|------|------|
| 경로 | `src/` | `frontend/src/` |
| 언어 | TypeScript (`.tsx`) | JavaScript (`.jsx`) |
| 배경 제거 위치 | `SlotList.tsx` 컴포넌트 | `useProducts.js` 훅 |
| 이미지 표시 | `Canvas.tsx` (CW=400, CH=500 좌표계) | `DraftCard.jsx` (Tailwind CSS 레이아웃) |
| 슬롯 개념 | `ImageSlotState` (width%, height%, x%, y%) | `Product` (image URL만 관리) |

### 8.2 포팅 내용

| 파일 | 변경 | 비고 |
|------|------|------|
| `frontend/src/config/backgroundRemoval.js` | **신규** | TS → JS 변환 (타입 어노테이션 제거) |
| `frontend/src/utils/cropToBoundingBox.js` | **신규** | TS → JS 변환 |
| `frontend/src/utils/removeBackground.js` | **신규** | TS → JS 변환, `calcSlotResize` 포함 (현재 미사용, 향후 사용 가능) |
| `frontend/src/main.jsx` | **수정** | `preload(BG_REMOVAL_CONFIG)` 추가 |
| `frontend/src/hooks/useProducts.js` | **수정** | `removeBgClientSide` → `removeBgPipeline` 교체 |

### 8.3 `useProducts.js` 변경 상세
...
async function removeBgClientSide(imageSrc) {
  const blob = await removeBackground(imageSrc);  // config 없음, 크롭 없음
  return URL.createObjectURL(blob);
}
// handleRemoveBackground 내부:
...
// handleRemoveBackground 내부:
const result = await removeBgPipeline(imageSrc);  // WebGPU + 크롭 포함
...
| 항목 | 이유 |
|------|------|
| `calcSlotResize` 호출 | `feature/initPage`에는 슬롯 좌표계가 없음 — 이미지는 Tailwind CSS `object-contain`으로 표시 |
| `bgRemoved` 플래그 | Product 모델에 해당 필드 없음 — 향후 필요 시 추가 |

### 8.5 이미지 표시 영역 수정 (`DraftShared.jsx`)

포팅 후 크롭된 이미지가 DraftCard에서 상단/하단이 잘리는 문제가 발생.

**원인:** `ImageFrame` 컨테이너가 `max-h-[45%]` + `overflow-hidden`으로 이미지를 물리적으로 잘라내고 있었음. 배경 있는 이미지에서는 눈에 띄지 않았으나, 크롭된 이미지(객체만 남은 상태)에서는 객체의 상단/하단이 잘리는 것이 명확히 드러남.

**수정 내용:**

| 항목 | Before | After | 이유 |
|------|--------|-------|------|
| 컨테이너 높이 | `max-h-[45%]` / `max-h-[55%]` | `h-[45%]` / `h-[55%]` | 고정 높이로 변경하여 자식 `max-h-full`이 참조 가능 |
| 오버플로우 | `overflow-hidden` | 제거 | 이미지 잘라내기 제거 |
| 이미지 래퍼 | `flex items-center justify-center` | `flex items-center justify-center h-full min-h-0` | `h-full`: 부모 높이 상속, `min-h-0`: flex 자식의 기본 `min-height: auto` 해제로 축소 허용 |

**결과:** 이미지가 컨테이너 높이에 맞춰 축소되어 전체가 표시되며, 컨테이너의 최종 높이는 기존과 동일하게 유지.

### 8.6 포팅 시 발견된 추가 문제

#### Tailwind CSS 미적용

**증상:** 페이지에 CSS가 전혀 적용되지 않음
**원인 1:** `08c91fc` 리팩터링 커밋에서 `postcss.config.js`가 삭제됨. Tailwind v4는 `@tailwindcss/postcss` 플러그인이 PostCSS 설정에 등록되어야 동작.
**해결:** `postcss.config.js` 복원
...
**원인 2:** `main.jsx`에서 `import './index.css'`의 위치가 `App.jsx` import 이전으로 이동됨. App 로드 이후 CSS가 적용되어야 정상 동작.
**해결:** `import './index.css'`를 `import App from './App.jsx'` 이후로 배치

### 8.7 빌드 검증

```
✓ built in 15.04s (에러 없음, Tailwind 경고 없음)
```
...
1. **정렬 기능**: 크롭 후 객체 정렬 (현재는 `object-contain`으로 자동 맞춤)
2. **WebGPU 성능 측정**: GPU 가속 적용 전후 실제 속도 비교 데이터 수집
3. **알파 임계값 설정 UI**: 이미지 특성에 따라 사용자가 임계값을 조정할 수 있는 옵션 검토
4. **DraftCard 레이아웃 개선**: 크롭된 이미지의 비율을 DraftCard 레이아웃에 반영 (`result.cropWidth`/`cropHeight` 활용)
