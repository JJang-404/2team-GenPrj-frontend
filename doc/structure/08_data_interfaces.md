...
extraInfo = {
  ...
  // 각 view* 는 InitPage 쪽에서 show* 로 들어옴
  showParkingCount, showPetFriendly, showPhone, showAddress,
  showIsNoKids, showSmokingArea, showHasElevator: boolean,
}
...
| `extraInfo.showParkingCount` | `.additionalInfo.viewParking` | show* → view* 일괄 |
| `extraInfo.showPetFriendly` | `.additionalInfo.viewPet` | |
| `extraInfo.showPhone` | `.additionalInfo.viewPhone` | |
| `extraInfo.showAddress` | `.additionalInfo.viewAddress` | |
| `extraInfo.showIsNoKids` | `.additionalInfo.viewIsNoKids` | |
| `extraInfo.showSmokingArea` | `.additionalInfo.viewSmokingArea` | |
| `extraInfo.showHasElevator` | `.additionalInfo.viewHasElevator` | |
...
### 6.2 `additionalInfoLabels` — 한국어 문자열 기반 "외래키"

단일 소스: [`editorFlow.ts:255-263`](../../react/src/modules/editing/utils/editorFlow.ts#L255-L263)

```ts
additionalInfoLabels = [
  '주차 공간 수', '애견 동반 가능 여부', '노키즈존',
  '흡연 구역 존재 여부', '엘리베이터 존재 여부',
  '전화번호', '주소',
] as const;
```
...
| 축 | 구조 | key/인덱스 |
|----|------|-----------|
| 데이터 축 | `HomeAdditionalInfo.{parkingSpaces, petFriendly, noKidsZone, ...}` | camelCase (TypeScript 필드명) |
| view* 축 | `HomeAdditionalInfo.{viewParking, viewPet, viewIsNoKids, ...}` | camelCase (TypeScript 필드명) |
| 표시 축 | `additionalInfoVisibility['한국어 레이블']` | **한국어 문자열** |

즉 "같은 항목" 이 **3가지 다른 식별자** 로 동시에 살아있다. 변환표:

| 데이터 필드 | view* 필드 | 한국어 레이블 (state key) |
|-------------|-----------|----------------------------|
| `parkingSpaces: number` | `viewParking` | `'주차 공간 수'` |
| `petFriendly: boolean` | `viewPet` | `'애견 동반 가능 여부'` |
| `noKidsZone: boolean` | `viewIsNoKids` | `'노키즈존'` |
| `smokingArea: boolean` | `viewSmokingArea` | `'흡연 구역 존재 여부'` |
| `elevator: boolean` | `viewHasElevator` | `'엘리베이터 존재 여부'` |
| `phoneNumber: string` | `viewPhone` | `'전화번호'` |
| `address: string` | `viewAddress` | `'주소'` |

변환이 일어나는 곳:
- **Data + view* → 한국어 key**: [`App.tsx handleStartFromHome`](../../react/src/modules/editing/App.tsx#L394-L409) 의 `seededVisibility` 객체
- **한국어 key → 아이콘/표시 텍스트**: [`utils/additionalInfo.ts`](../../react/src/modules/editing/utils/additionalInfo.ts) 의 `getAdditionalInfoIcon` / `getAdditionalInfoDisplayText` switch
- **한국어 key → 요소 생성**: [`editorFlow.createAdditionalInfoElements(projectData, label, visibility)`](../../react/src/modules/editing/utils/editorFlow.ts#L1088-L1141)
...
| `info-${slugInfoLabel(label)}-icon\|text` | 부가정보 아이콘/텍스트 | `createAdditionalInfoElements` |
| `custom-text-${ts}` | 사용자 추가 텍스트 | `createCustomTextElement` |
| `custom-image-${ts}` | 사용자 업로드 이미지 | `createCustomImageElement` |

`slugInfoLabel(label)` 은 한국어 레이블 → ASCII slug 변환 (공백 → '-'). 전체
key 검사 시 이 변환이 양쪽 끝에 일관되게 들어가야 한다.
...
|-----------|------------------------|
| `HomeAdditionalInfo` 필드 추가 | (1) `additionalInfoLabels` 배열, (2) `buildEditingPayload.additionalInfo` 블록, (3) `handleStartFromHome.seededVisibility`, (4) `additionalInfo.getAdditionalInfoIcon/DisplayText`, (5) `AdditionalInfoSection` 체크박스 렌더 |
