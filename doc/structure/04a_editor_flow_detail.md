...
Line 252-253 import additionalInfo display helpers + cloneTemplateElements
Line 255-263 additionalInfoLabels (7개 한국어 레이블)
...
Line 491-493 slugInfoLabel                                          ← index util
...
| `createAdditionalInfoElements(projectData, label, visibility?)` | 3개 인자 | `EditorElement[]` (0-2개) | 순수 |
| `shouldShowAdditionalInfoText(projectData, label, visibility?)` | 3개 인자 | `boolean` | 순수 |
| `shouldShowAdditionalInfoIcon(projectData, label, visibility?)` | 3개 인자 | `boolean` | 순수 |
| `buildGuideSummary(projectData, template)` | 2개 인자 | `string` | 순수 |
| `slugInfoLabel(label)` | `string` | `string` (1-based index) | 순수 |
...
- **부가정보**: `additionalInfoLabels.forEach` 로 7 개 label 모두 시도하지만,
  `createAdditionalInfoElements` 내부에서 visibility × 데이터 유무 2 조건을
  모두 만족해야 실제 element 가 반환됨 (§ 7).
...
**7 개 부가정보 label 을 화면 하단에 배치**하는 전담 계산기. 호출 컨텍스트
2 개 소비자: `createAdditionalInfoElements` (element 생성 시) 와
`applyDraftLayoutVariant` / `toggleAdditionalInfoElements` (재배치 시).
...
> `FOOTER_ICON_LABELS` 는 정확히 5개 (`주차 공간 수`, `애견 동반 가능 여부`,
> `노키즈존`, `흡연 구역 존재 여부`, `엘리베이터 존재 여부`). 아이콘은 우측
> 정렬이므로 영역 폭은 최대치 (5 개 모두 on) 기준으로 고정 예약해, 텍스트
> 영역 `TEXT_W` 가 아이콘 on/off 에 따라 흔들리지 않도록 한다.

### 5.2 FOOTER_TEXT_LABELS vs FOOTER_ICON_LABELS

두 배열에 **`주차 공간 수`가 중복**된다. 주차만 "텍스트 + 아이콘 양쪽" 을
가지는 2중 항목이기 때문.

```
FOOTER_TEXT_LABELS = ['전화번호', '주소', '주차 공간 수']           // 3개
FOOTER_ICON_LABELS = ['주차 공간 수', '애견…', '노키즈존', '흡연…', '엘베…']  // 5개
```

### 5.3 알고리즘

```
1. activeTextLabels = TEXT_LABELS 중 shouldShowText 통과한 것만
   lineCount = activeTextLabels.length
   footerH   = max(lineCount, 1) * LINE_H
   footerTopY = 96 - footerH                 ← 위로 자라는 스택

2. activeIconLabels = ICON_LABELS 중 shouldShowIcon 통과한 것만
   usedIconWidth = n * ICON_SIZE + (n-1) * ICON_GAP
   iconLeftmostX = FOOTER_X + FOOTER_W - usedIconWidth   ← 우측 정렬
   iconY = 96 - ICON_SIZE

3. 텍스트 preset: activeTextLabels 만 y 좌표 배정 (textIdx 순회),
                 비활성은 EMPTY_RECT.

4. 아이콘 preset: activeIconLabels 만 좌→우 순서로 iconLeftmostX + idx*ICON_STEP,
                  비활성은 EMPTY_RECT.

5. 반환: Record<label, { text, image }>  (7 label 모두 key 포함)
```
...
사용자 토글로 label on/off → lineCount 변화 → footerTopY 이동. 예:
...
606-712  elements.map((element) => {
           ...
           case id matches /^info-text-\d+$/:         → computeFooterPresets[label]
           case id matches /^info-image-\d+$/:        → computeFooterPresets[label]
           default:                                    → element 그대로
         })
```
...
`info-text-{slug}` / `info-image-{slug}` id 에서 **숫자 slug → label
역매핑**. slug 는 `slugInfoLabel(label)` (line 491-493) 이 만드는
`additionalInfoLabels.indexOf(label) + 1`. 즉 1-based 인덱스이므로 `-1` 로
되돌린다.
