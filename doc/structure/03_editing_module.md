...
handleStartFromHome(data)
  ...
  ├─ setProjectData(baked)
  │
  ├─ seededVisibility = {                         ← bridge payload 의 view* 를 그대로 identity copy
  ...
  │   } satisfies Record<AdditionalInfoKey, boolean>
...
| | `additionalInfoVisibility` | `Record<AdditionalInfoKey, boolean>` | 부가정보 on/off — **key 는 `view*` (`'viewParking'` 등)**. InitPage/bridge/editing 3축이 같은 키 공유 |
...
### 3.2 `additionalInfoVisibility` 계약 — `view*` key 규약

이 state 는 `Record<AdditionalInfoKey, boolean>` 으로 타입 강화되어 있으며,
`AdditionalInfoKey = 'viewParking' | 'viewPet' | 'viewNoKids' |
'viewSmoking' | 'viewElevator' | 'viewPhone' | 'viewAddress'` 의 7개
유니온이다. InitPage `extraInfo.view*` / bridge `projectData.additionalInfo.view*` /
editing `additionalInfoVisibility[view*]` 세 축이 **동일한 key** 를 공유
하므로 rename 없는 identity pass-through 가 성립한다.

**규약의 출처**:

| 위치 | 코드 | 의미 |
|------|------|------|
| [`additionalInfo.ts`](../../react/src/modules/editing/utils/additionalInfo.ts) | `ADDITIONAL_INFO_ITEMS = [{ viewKey, label, dataField }, ...] as const`<br/>`ADDITIONAL_INFO_KEYS = ADDITIONAL_INFO_ITEMS.map(i => i.viewKey)`<br/>`type AdditionalInfoKey = typeof ADDITIONAL_INFO_KEYS[number]` | **Single source of truth** — 7개 `view*` 키, UI 레이블, `HomeAdditionalInfo` 데이터 필드 참조가 한 테이블에 묶임 |
| [`App.tsx handleToggleInfoItem`](../../react/src/modules/editing/App.tsx) | `(viewKey: AdditionalInfoKey) => setAdditionalInfoVisibility(prev => ({ ...prev, [viewKey]: !prev[viewKey] }))` | 토글은 `viewKey` 기반 |
| [`App.tsx infoItems`](../../react/src/modules/editing/App.tsx) | `ADDITIONAL_INFO_ITEMS.map(({viewKey, label}) => ({ viewKey, label, visible: additionalInfoVisibility[viewKey] ?? false }))` | 사이드바에 `{viewKey, label, visible}` 3-tuple 전달 — label 은 UI 표시 전용 |
| [`editorFlow.ts createAdditionalInfoElements 루프`](../../react/src/modules/editing/utils/editorFlow.ts) | `ADDITIONAL_INFO_KEYS.forEach(viewKey => createAdditionalInfoElements(projectData, viewKey, visibility))` | canvas 요소 생성 시 `viewKey` 로 visibility 조회 |
| [`editorFlow.shouldShowAdditionalInfoText/Icon`](../../react/src/modules/editing/utils/editorFlow.ts) | `visibility[key]` 직접 조회 (`key: AdditionalInfoKey`) | 렌더 판단 |

UI 에 표시되는 한국어 레이블은 `getAdditionalInfoLabel(viewKey)` 헬퍼로
동일 테이블에서 도출되며, EditorElement.label 내부 값으로만 저장된다.
state key 로는 사용되지 않는다.

### 3.3 InitPage / bridge / editing 3축 1:1 pass-through

세 축이 모두 `view*` (AdditionalInfoKey) 를 공유하므로 경계에서 rename 이
일어나지 않는다. 변환 대신 `Boolean()` 감싸기로 `undefined` 를 `false` 로
좁히기만 하면 된다.

| 축 | 위치 | 표현 |
|----|------|------|
| InitPage state | `design.DEFAULT_EXTRA_INFO` / `ExtraInfoSection.jsx` | `extraInfo.viewParking`, … |
| Bridge payload | `initPage/utils/editingBridge.buildEditingPayload` | `projectData.additionalInfo.viewParking`, … |
| Editing state | `editing/App.additionalInfoVisibility` | `additionalInfoVisibility['viewParking']`, … |
| UI 표시 레이블 | `getAdditionalInfoLabel(viewKey)` | `'주차 공간 수'`, … (렌더 전용) |

누락된 `view*` 필드는 `Boolean(undefined) === false` 로 자동 off 처리
(§ 2.1 의 `Boolean(info.*)` 감싸기).

> **주의 — 부가정보 플래그 추가 시**:
> 1. [`editing/utils/additionalInfo.ADDITIONAL_INFO_ITEMS`](../../react/src/modules/editing/utils/additionalInfo.ts) 에 `{ viewKey, label, dataField }` 추가
> 2. [`types/home.HomeAdditionalInfo`](../../react/src/modules/editing/types/home.ts) 에 `view*` + 데이터 필드 추가
> 3. [`initPage/utils/editingBridge.buildEditingPayload`](../../react/src/modules/initPage/utils/editingBridge.js) 의 `additionalInfo` 블록에 `viewX: Boolean(extraInfo.viewX)` identity pass-through 추가
> 4. [`initPage/constants/design.DEFAULT_EXTRA_INFO`](../../react/src/modules/initPage/constants/design.js) 에 `viewX` 초기값 seed
> 5. [`App.tsx handleStartFromHome`](../../react/src/modules/editing/App.tsx) 의 `seededVisibility` 객체에 identity copy 추가
> 6. [`utils/additionalInfo.getAdditionalInfoIcon/DisplayText`](../../react/src/modules/editing/utils/additionalInfo.ts) 에 아이콘/표시 텍스트 케이스 추가
>
> 한 곳이라도 누락하면 타입 에러 또는 런타임 off 불일치가 발생한다.
...
| `slugInfoKey(viewKey)` | `AdditionalInfoKey` → slug (info-N-icon/text id 생성용) |
| `ADDITIONAL_INFO_KEYS` (from `utils/additionalInfo.ts`) | 상수 배열(7개 `view*` 키 — `additionalInfoVisibility` key 규약의 출처. § 3.2) |
...
| 새 부가정보 플래그 | `utils/additionalInfo.ADDITIONAL_INFO_ITEMS` 에 `{viewKey, label, dataField}` 추가 + `types/home.HomeAdditionalInfo` (`view*` + 데이터 필드) + `initPage/utils/editingBridge.buildEditingPayload` 에 `viewX: Boolean(extraInfo.viewX)` 1:1 pass-through + `initPage/constants/design.DEFAULT_EXTRA_INFO` seed + `App.tsx handleStartFromHome` 의 `seededVisibility` identity copy + `additionalInfo.getAdditionalInfoIcon/DisplayText` | § 3.2-3.3 참조. 세 축 모두 동일한 `view*` 키 공유. 아이콘 매핑 누락 시 자동 fallback 없음 |
