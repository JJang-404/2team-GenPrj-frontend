    ...
    def _merge_dicts(self, base: dict, updates: dict) -> dict:
        ...
        for key, value in updates.items():
            if isinstance(value, dict) and isinstance(merged.get(key), dict):
                ...
                # 리스트 내부 요소가 딕셔너리이고 content_key나 id를 공유하면 병합 처리
                if value and isinstance(value[0], dict) and merged.get(key) and isinstance(merged[key][0], dict):
                    merged_list = deepcopy(merged[key])
                    for new_item in value:
                        found = False
                        for old_item in merged_list:
                            c_key = new_item.get("content_key") or new_item.get("id")
                            if c_key and c_key == (old_item.get("content_key") or old_item.get("id")):
