...
import { exportEditorAsDataUrl } from './utils/exportCanvas';
import { downloadDataUrl, readFileAsDataUrl } from './utils/file';
...
export default function App() {
  ...
  const handleGenerateBackgrounds = async () => {
    ...
    try {
      ...
      // [Case B] AI 배경 생성 모드 (순수 생성)
      console.log('[Editing] AI 배경 순수 생성 시작');
      ...
      if (res.ok && res.blobUrl) {
        const newCandidate = {
          ...
          note: '업종과 프롬프트를 기반으로 생성된 고품질 배경입니다.',
        ...
        setStep('background');
        console.log('[Editing] AI 배경 생성 완료 및 적용');
      } else {
  ...
  const handleExportProject = async () => {
    if (!selectedBackground && backgroundMode === 'ai-image') return;
    setGenerating(true);
    setError(null);
    try {
      const ratio = projectData?.options.ratio ?? '4:5';
      const dataUrl = await exportEditorAsDataUrl(renderElements, selectedBackground, ratio);
      downloadDataUrl(dataUrl, `ad_${Date.now()}.png`);
    } catch (err) {
      console.error('[Export] 저장 실패:', err);
