...
function heuristicTranslation(promptKo: string, backgroundMode: string, guideSummary = '') {
  ...
  const industry = (info.basicInfo?.industry || '').toLowerCase();
  const storeName = info.basicInfo?.storeName || '';

  const industryKeywords = [
    industry.includes('카페') || industry.includes('커피') ? 'cafe interior with coffee machines and tables' : '',
    industry.includes('식당') || industry.includes('음식') ? 'restaurant dining area, elegant table setting' : '',
    industry.includes('한식') ? 'traditional korean restaurant atmosphere' : '',
    industry.includes('바') || industry.includes('주점') ? 'moody bar counter, lounge lighting' : '',
    industry.includes('베이커리') || industry.includes('빵') ? 'bakery shelf with warm lights' : '',
  ].filter(Boolean).join(', ');

  const promptKeywords = [
    lower.includes('봄') ? 'spring season, cherry blossoms, soft sunlight, pastel tones' : '',
    lower.includes('여름') ? 'summer vibe, bright daylight, tropical plants, refreshing atmosphere' : '',
    lower.includes('가을') ? 'autumn mood, maple leaves, warm sunset lighting' : '',
    lower.includes('겨울') ? 'winter theme, soft snow, cozy indoor lighting' : '',
    lower.includes('말차') || lower.includes('녹차') ? 'matcha theme, zen garden background' : '',
    lower.includes('라떼') ? 'milky textures, cafe backdrop' : '',
    lower.includes('초코') || lower.includes('초콜릿') ? 'dark chocolate aesthetic' : '',
  ].filter(Boolean).join(', ');

  const modeHint: Record<string, string> = {
    solid: 'clean solid-color composition',
    gradient: 'smooth gradient backdrop',
    pastel: 'graphic poster style with clean blocks',
    'ai-image': 'photorealistic advertising photography, professional studio lighting, cinematic depth of field',
  };

  return [
    `Product advertisement background for ${storeName || 'brand'}`.trim(),
    industryKeywords,
    promptKeywords,
    guideSummary ? `layout: ${guideSummary}` : '',
    modeHint[backgroundMode] || 'commercial backdrop',
    '8k resolution, highly detailed, realistic materials',
    'background only, no extra people, no extra hands, no floating objects',
  ].filter(Boolean).join(', ');
}
