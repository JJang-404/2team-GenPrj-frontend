 ...
 *          url: data URL of the cropped image
 *          width/height: pixel dimensions of the cropped region
 *          If no opaque pixels found, returns original image as data URL with source dimensions.
 */
export async function cropToBoundingBox(blobUrl) {
  ...
  if (maxX < 0) {
    ...
    return { url: fallbackCanvas.toDataURL('image/png'), width: srcW, height: srcH };
  }
